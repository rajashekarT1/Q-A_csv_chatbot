...
#                 generated_text = json_response[0]["generated_text"]
#                 st.write("### Generated Response:")
#                 st.markdown(f'<div style="border: 1px solid #ddd; padding: 10px; background: #f9f9f9;">{generated_text}</div>', unsafe_allow_html=True)
#             except requests.exceptions.RequestException as e:
#                 st.error(f"Request failed: {e}")
#             except (KeyError, IndexError, TypeError) as e:
#                 st.error(f"Error processing the response: {e}. Check the API response format.")
...
# headers = {"Authorization": "Bearer your api key"}  # Replace with your Hugging Face API token
